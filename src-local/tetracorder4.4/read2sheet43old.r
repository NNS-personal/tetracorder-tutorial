...
#########################################################################
	integer*2 i2sht(maxpi2,imaxch)   # buffer to hold I*2 data
#	integer*2 i2sht(1,4852)     # buffer to hold I*2 data

######## select one of the following:
#########################################################################
	character*maxpi4 chibuff(imaxch)  # buffer to read I*2 data
#	character*8000 chibuff(4852) # buffer to read I*2 data

        equivalence     (i2sht,chibuff)

######## select one of the following:
#########################################################################
	real*4	specdat(imaxch,maxpix)   # extracted spectra in BIP format
#	real*4	specdat(4852,1)   # extracted spectra in BIP format

	integer*4 zel

#variables for ENVI data-cube reads
      character*maxpi4 chiline       # buffer to read all data
      real*4 r2line(4000)           # buffer to read real*4 data
      equivalence (r2line, chiline)
