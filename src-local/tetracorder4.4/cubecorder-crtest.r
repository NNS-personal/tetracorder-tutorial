...
# DEBUG:
			write (ttyout,*) 'DEBUG: filhdr=',filhdr,
                                        'reclen=',reclen,
					'rechdr=',rechdr,
					'lines=',dy,
                                        'samples=',dx,
			...
			call system ('/usr/local/securebin/logtetracorder' // chtmp2(1:lnb(chtmp2)) // char(0))
...
# Get nth variable value
	...
	nth = nint(x)
     write(*,*) 'nth var & (i, x, il) = ', nth, i, x, il
...
# Get diagnostics flag
	...
	diaflg = nint(x)
     write(*,*) 'diaflg & (i, x, il) = ', diaflg, i, x, il
