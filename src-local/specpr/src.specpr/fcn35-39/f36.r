...
#ccc            range.  F36 is setup for the actual routine: bandzmp
#ccc            F36, bdmzset, and bandzmp are equivalents the the f42, bandmp
#ccc            algoirthms except 1.0 is subtracted from the continuum removed
#ccc            spectra.  That way, the least suqares is only a scaling, no offset
#ccc
#ccc            NOTE: this algorithm was used to test if it would produce better
#ccc            results than the f42 algirithm.  It does not.  Fits are equally
#ccc            good.
#ccc
...
#   
	...
	call bdmzset (dataa,data,nch1,nch2,nch3,nch4,
...
#           conref = continuum reflectance of observed spec at band minimum
	...
	iypxl = 0
	ictrol = 1
	call bandzmp (dataa,rflibc,datab,nch1,nch2,nch3,nch4,ictrol,minch,
...
#	b = yintcp
#
## correct remaining conrinuum
#	if (nch1 < nch2) {
#		do i = nch1, nch2-1 {
#			if (dataa(i) == delpt || rflibc(i) == delpt ||
#							datab(i) == delpt) {
#				datsc2(i) = delpt
#			} else {
#				contin = a * dataa(i) + b
#				if (abs(contin) > 0.1e-20) {
#					datsc2(i) = datab(i) / contin
#				} else {
#					datsc2(i) = delpt
#				}
#			}
...
410	format (/,'OUTPUT Selection:',//,
		...
		' or type  o  to save CONTINUUM REMOVED OBSERVED spectrum')
	call crtin
	i=1
	call wjfren (i,x,il)
	if (il==ihx || il==ihe) {
...
#	determine history
	...
	write(mhist(149:222),489) nch1, nch2, nch3, nch4
...
115     format ( ' Function f36: Band Mapping:',//,
'     This function fits a reference spectrum over a given wavelength', /,
'          (channel) interval.' //,
'     opearing on : ',a,i4,':',a,//,
...
487   format('**f36: wave set: ',a,' r',i7,
		'  fit =',f7.4,a)
488   format('**f36: lib stretch param (xk)=', f10.4,
		'    continuum (conrem)=',f8.5,'  ')
489   format('**f36: continuum channels:',i5,' ',i5,' ',i5,' ',i5,
		' ',20(' '))

506     format ('f36:lib=',a8,' r',i7,' obs=',a8,' r',i7,' dpth=',f6.4)

507     format ('f36:',a8,' r',i7,' fit lib=',a8,' r',i7,' dpth=',f6.4)
