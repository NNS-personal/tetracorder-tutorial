...
#RED 
	...
	dimension       idsum(128),ifels(128)
	dimension       inumd(4864),inume(4864),error2(4864)
...
#
	...
	if ((rcl != 19456 & lopen) | (.not. lopen)) {
		close (addlun)
		open(addlun,status='scratch',access='direct',
			recl=19456,form='unformatted',iostat=ier)
		if (ier != 0) {
...
#
	...
	if (isumav!=ihs)
		inm = 'avg   '
	iflag = 0
	ichar = manhst(inm,6,idsum,ifels,js,mhist,iflag,constx)
	if (ichar<=40) {
