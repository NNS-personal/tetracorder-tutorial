/************************************************************************/
/*                                                                      */
/*  Randall Dailey 3/1/2004                                             */
/*    Added call to byteorder to perform swap if required               */
/*                                                                      */
/************************************************************************/
...
/* RED if running on little endian system, call byteorder to output in big endian format */
...
/* RED Zero the flag since cannot guarantee its init value */
    ...
    switch(check_bit(label->icflag,1)) {
		...
		/* NOTE: elements of tmpl struct are
...
/* NOTE: look to pass to write_record do_linux or big_endian 
         as well as whether data or text as noted by check above  */

    write_record(fd, i, label);

    count = 0;
    size -= offset;
    /* Set the continued bit */
    set_bit(tmpl.icflag,0,1);
    saveicflag=tmpl.icflag;
    while(size > 0) {
	tmpl.icflag=saveicflag;
