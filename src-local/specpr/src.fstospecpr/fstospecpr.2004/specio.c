/******************************************************************************
Modifications:
  - Randall Dailey 24 May 2004
    Set infile and outfile to NULL to ensure later value check will always work.
...
char **av;
{
    ...
    char *ptr;
    /* NOTE:
         chkendian returns 1 if running on a little endian platform (i.e. linux)
         or 0 if running on a big endian platform
    */
    int do_linux = chkendian();
/*  printf("do_linux %d\n",do_linux);
    exit(0); */


/* RED - Explicitly set infile and outfile to NULL else check below likely will not equate */
    infile = NULL;
    outfile = NULL;
    while (--ac) {
        av++;
...
/* RED Added the following initialization of the label structure to ensure clean */
