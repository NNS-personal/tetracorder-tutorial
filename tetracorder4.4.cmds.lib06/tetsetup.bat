...
REM ----- assign SYSTEM VARIABLES from command-line argument list ------
   ...
   rem set the current drive name
   rem (OLD)      set drive-data=C:
      set drive-current=%CD:~0,2%
      set drive-data=%drive-current%
      set cube-dos=%cube%

   rem set drive-data for cube drive when used (current drive or other drive-device)
   rem remove <drive> from cube string if present
      if %cube:~1,1%==: (
         set drive-data=%cube:~0,2%
         set cube=%cube:~2%
      )

REM Kludge for now; Cygwin requires either backslashes or dos-drive (ex. c:) at start of DOS path
REM      (for Windows versions of Tetracorder & SpecPR; Davinci [MinGW] uses unix form) 
      if %cube-dos:~0,1%==/ set cube-dos=%drive-data%%cube%
...
REM ----- assign remaining SYSTEM VARIABLES ----------------------------
   ...
   rem copy and configure restart (r1) file
      ...
      rem   (substitute back-slashes for any (all) foreward-slashes)
         set iyfl_org-subset-dos=%iyfl_org-subset:/=\%

         if exist %iyfl_org-subset-dos% (
            rem spectral library path is correct in variable "iyfl"
            set iyfl_new-subset=%iyfl_org-subset%
         ) else (
            rem use variable "speclib-path" defined at top of this file with filename below
            for %%A in ("%iyfl_org-subset%") do (
               set iyfl_org-subset-path=%%~dpA
               set iyfl_org-subset-filename=%%~nxA
            )
            rem echo spectral path = %iyfl_org-subset-path%
            rem echo spectral filename = %iyfl_org-subset-filename%
            set iyfl_new-subset=%speclib-path%%iyfl_org-subset-filename%
         )
         rem echo iyfl_new-subset set to %iyfl_new-subset%

         if not exist %iyfl_new-subset% (
            rem libray not found, try defining path relative to current directory
            set iyfl_new-subset=%parent-source-path%%iyfl_org-subset%
               rem check for beginning slash
            if not %iyfl_org-subset:~0,1%==/ set iyfl_new-subset=%parent-source-path%/%iyfl_org-subset%
         )
      ...
      REM Kludge for now; Cygwin requires either backslashes or dos-drive (ex. c:) at start of DOS path
      REM      (for Windows versions of Tetracorder & SpecPR; Davinci [MinGW] uses unix form) 
         if %iyfl_new-subset:~0,1%==/ set iyfl_new-subset=%drive-current%%iyfl_new-subset%

      rem trim iyfl string to a 77 character line as found in the 'r1' file
         set iyfl_new-subset=%iyfl_new-subset:~0,77%

      rem   (substitute back-slashes for any (all) foreward-slashes)
         set iyfl_new-subset-dos=%iyfl_new-subset:/=\%

      rem then, test for new path and spectral-library-file:
         if NOT exist %iyfl_new-subset-dos% (
            echo spectral-library-file set to %iyfl_new-subset-dos%
...
REM show variable values to screen
   ...
   echo current drive: . . . . . . . . . . . . . %drive-current%
   echo datacube drive:  . . . . . . . . . . . . %drive-data%
...
REM Kluge - use DOS drive if cube path started with '/' for Cygwin Tetracorder version
   ...
   rem remove carriage returns in test file
      %binpc%\dos2unix %dir%\%start%
